...
namespace
{
	...
	TEST(HorizontalAlignmentElement2DClothoid, getPosition)
	{
		...
		HorizontalAlignmentElement2DClothoid_old::Ptr c =
	...
	TEST(HorizontalAlignmentElement2DClothoid, getPosition3)
	{
		...
		buw::HorizontalAlignmentElement2DClothoid::Ptr c =
	...
	{
		buw::LandXmlImport parser(filename.c_str());
		...
		{
			buw::IAlignment3D::Ptr a3 = alignments[k];

			if (a3->getType() == buw::e3DAlignmentType::e2DBased)
			{
				buw::Alignment2DBased3D::Ptr a = std::static_pointer_cast<buw::Alignment2DBased3D>(a3);
				...
				for (int i = 0; i < a->getHorizontalAlignment()->getAlignmentElementCount(); i++)
				{
					...
					if (he->getAlignmentType() == buw::eHorizontalAlignmentType::Clothoid)
					{
						...
						auto computedEndPoint = buw::HorizontalAlignmentElement2DClothoid_old::computeEndPoint(
							startPoint,
							buw::calculateAngleBetweenVectors(buw::vector2d(1.0, 0), piPoint - startPoint),
							L,
							RadiusStart,
							!clockwise,
						...
						/*
						if (buw::distance(computedEndPoint, endPoint) > 0.1)
						{
						std::cout << "muh" << std::endl;

						auto computedEndPoint = buw::HorizontalAlignmentElement2DClothoid::computeEndPoint(
						startPoint,
						buw::calculateAngleBetweenVectors(buw::vector2d(1.0, 0), piPoint - startPoint),
						L,
						RadiusStart,
