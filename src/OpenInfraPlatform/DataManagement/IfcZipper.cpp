...
OpenInfraPlatform::DataManagement::IfcZipper::IfcZipper(QObject* parent,QString path,QString tempName, bool operation) {
	if ((tempName.contains(QString(".ifczip")) && operation == DEFL)
		...
		BLUE_LOG_STREAM_EX("IfcZipExport", buw::eLogSeverityLevel::Error) << "File has false Data Format!";
	}
...
int OpenInfraPlatform::DataManagement::IfcZipper::createLocalFileHeader(localzipheader* locfilehdr, Byte* inBuffer, uLong inFileSize) {
	if (locfilehdr == NULL || inBuffer == NULL || inFileSize == NULL) {
		BLUE_LOG_STREAM_EX("IfcZip Export", buw::eLogSeverityLevel::Error) << "Received null as a parameter";
		return -1;
	}
	else {
...
int OpenInfraPlatform::DataManagement::IfcZipper::writeLocalFileHeader(localzipheader* header) {
	if (!ofs.is_open()) {
		BLUE_LOG_STREAM_EX("IfcZip Export", buw::eLogSeverityLevel::Error) << "Output filestream is closed";
		return -1;
	}

	if (header == NULL) {
		BLUE_LOG_STREAM_EX("IfcZip Export", buw::eLogSeverityLevel::Error) << "header is null";
		return -2;
	}
...
int OpenInfraPlatform::DataManagement::IfcZipper::createCentralDirectoryRecord(localzipheader* header, CentralDirectoryRecord* cdr) {
	...
	if (cdr == NULL) {
		BLUE_LOG_STREAM_EX("IfcZip Export", buw::eLogSeverityLevel::Error) << "cdr is null";
		return -2;
	}
...
void OpenInfraPlatform::DataManagement::IfcZipper::parseLocalFileHeader(localzipheader* header) {
	...
	if (!ifs.is_open()) {
		BLUE_LOG_STREAM_EX("IfcZip Export", buw::eLogSeverityLevel::Error) << "Input filestream is closed";
		return;
	}
	else {
		ifs.seekg(0, ifs.beg);
...
void OpenInfraPlatform::DataManagement::IfcZipper::init(localzipheader* header) {
	...
	if (mode == DEFL) {
		...
		if (ifs.good() && ofs.good()) {
			...
			BLUE_LOG_STREAM_EX("IfcZip Export", buw::eLogSeverityLevel::Error) << "Initializing the filestreams failed for deflate";
		}
		...
		if (!ofs.good() || !ifs.good()) {
			BLUE_LOG_STREAM_EX("IfcZip Export", buw::eLogSeverityLevel::Error) << "Initializing the filestreams failed for inflate";
		}
	}
...
int OpenInfraPlatform::DataManagement::IfcZipper::createEndOfCentralDirectoryRecord(EndOfCentralDirectoryRecord* eocdr, CentralDirectoryRecord* cdr){
	...
	if (eocdr == NULL || cdr == NULL) {
		BLUE_LOG_STREAM_EX("IfcZipExport", buw::eLogSeverityLevel::Error) << "EOCDR or DCR is null";
		state = -1;
	}
...
int OpenInfraPlatform::DataManagement::IfcZipper::writeEndOfCentralDirectoryRecord(EndOfCentralDirectoryRecord* eocdr) {
	int state = 0;
	if (eocdr == NULL) {
		BLUE_LOG_STREAM_EX("IfcZipExport", buw::eLogSeverityLevel::Error) << "Received NULL as EOCDR";
		state = -3;
	}	
	if (ofs.bad() || ofs.fail()) {
		BLUE_LOG_STREAM_EX("IfcZipExport", buw::eLogSeverityLevel::Error) << "Output Filestream has badbit or failbit set before writing EOCDR";
		state = -1;
	}
	ofs.write((char*)eocdr, 22);
	if (ofs.bad() || ofs.fail()) {
		BLUE_LOG_STREAM_EX("IfcZipExport", buw::eLogSeverityLevel::Error) << "Output Filestream has badbit or failbit set after writing EOCDR";
		state = -2;
	}
	return state;
}
...
int OpenInfraPlatform::DataManagement::IfcZipper::removeFile() {
	...
	if (result != 0) {
		BLUE_LOG_STREAM_EX("IfcZipExport", buw::eLogSeverityLevel::Error) << "Deleting temporary file failed";
	}
	Q_EMIT fileDeleted();
	return result;
}