...
#include "OpenInfraPlatform/Infrastructure/Export/SVGExportAdvanced.h"
						...
						if(domElement.tagName() == "Alignment2D")
						{
							...
							for(int i=0; i<horiChildNodes.length(); i++)
							{
								...
								else
								{
									...
									buw::ReferenceCounted<buw::HorizontalAlignmentElement2DClothoid> clothoid = std::make_shared<buw::HorizontalAlignmentElement2DClothoid>(start, startDirection, startCurvature, !clockwise, clothoidConstant, entry, length);
...
void OpenInfraPlatform::DataManagement::Data::importJob(const std::string& filename)
{
	...
	else if (buwstrFilename.toLower().endsWith(".rnd"))
	{
		...
		importer_ = new buw::RoadXMLImport(filename);
	}
	...
	else if (buwstrFilename.toLower().endsWith(".ifc") || buwstrFilename.toLower().endsWith(".stp"))
	{
		...
		else if (ifcSchema == IfcPeekStepReader::IfcSchema::IFC_4x1)
		{
			...
			importer_ = new buw::IfcAlignmentImport(filename);
		}
		else if (ifcSchema == IfcPeekStepReader::IfcSchema::IFC_ROAD)
		{
			...
			importer_ = new buw::IfcRoadImport(filename);
		}
		else
		{
	...
	{
		importOSMJob(filename, buw::OSMImport::getDefaultFilter(), 2);
	}
}
...
void OpenInfraPlatform::DataManagement::Data::importOSMJob(const std::string& filename, const std::vector<std::string>& filter, int mode)
{
	...
	importer_ = new buw::OSMImport(filename, filter, mode);
}
...
void OpenInfraPlatform::DataManagement::Data::jobFinished(int jobID, bool completed)
{
	...
	if (importer_)
	{
		flag = flag | ChangeFlag::AlignmentModel | ChangeFlag::DigitalElevationModel | ChangeFlag::Preferences;
	...
	if (tempPointCloud_)
	{
		...
		if (merge_)
		{
			...
			pointCloud_->minPos = buw::getMinimizedVector(pointCloud_->minPos, tempPointCloud_->minPos);
			pointCloud_->maxPos = buw::getMinimizedVector(pointCloud_->maxPos, tempPointCloud_->maxPos);
		}
		else
		{
...
void OpenInfraPlatform::DataManagement::Data::exportIfcRoadTUMProposalJob(const std::string& filename)
{
	...
	new buw::IfcRoadExport(desc, alignmentModel_, digitalElevationModel_, filename);
}
...
void OpenInfraPlatform::DataManagement::Data::exportIfcAlignmentJob(const buw::ifcAlignmentExportDescription& desc, const std::string & filename)
{
	...
	new buw::IfcAlignmentExport(desc, alignmentModel_, digitalElevationModel_, filename);
}
...
void OpenInfraPlatform::DataManagement::Data::exportSVGAdvancedJob(const std::string& filename)
{
	...
	new buw::SVGExportAdvanced(alignmentModel_, digitalElevationModel_, filename);
}
...
void OpenInfraPlatform::DataManagement::Data::exportSVGJob(const std::string& filename)
{
	...
	buw::SVGExport(alignmentModel_, digitalElevationModel_, filename);
}
...
void OpenInfraPlatform::DataManagement::Data::exportOkstraJob(const std::string& filename, const std::string& version)
{
	...
	buw::OkstraExport(stoi(major), stoi(minor), alignmentModel_, digitalElevationModel_, filename);
}
...
void OpenInfraPlatform::DataManagement::Data::createExcelReportJob(const std::string& filename, bool useDegree)
{
	...
	exportIfcAlignmentJob(desc, ifcFilename);
	exportLandXMLJob(landXMLFilename);

	buw::ExcelReport ec(filename.c_str(), landXMLFilename.c_str(), ifcFilename.c_str());
}
...
void OpenInfraPlatform::DataManagement::Data::exportLandXMLJob(const std::string& filename)
{
	...
	new buw::LandXMLExport(alignmentModel_, digitalElevationModel_, filename);
}
