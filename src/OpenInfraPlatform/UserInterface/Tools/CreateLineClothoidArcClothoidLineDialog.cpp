...
void OpenInfraPlatform::UserInterface::CreateLineClothoidArcClothoidLineDialog::on_pushButtonGenerate_clicked()
{
	...
	RelM1 = RelCloEnd1 + R* buw::createRotationZ22d(M_PI / 2 + tau[0])*x_axis;
	RelM2 = RelCloEnd2 + R* buw::createRotationZ22d(M_PI / 2 + tau[1])*x_axis;
	for (int Groupnumber = 0; Groupnumber < points_.size() - 2; Groupnumber++)
	{
		give3Points(U, V, W, LineDir[0], LineDir[1], angle, dummy[0], dummy[1], points_[Groupnumber].xy(), points_[Groupnumber + 1].xy(), points_[Groupnumber + 2].xy(), offset_.xy());
		gamma = M_PI - angle;
		...
		center = V + t1*LineDir[0] + RelM1.y()* buw::createRotationZ22d(-sig*M_PI / 2)* LineDir[0];
		start = V + (T1 - RelCloEnd1.x())*LineDir[0] + RelCloEnd1.y()*buw::createRotationZ22d(-sig*M_PI / 2)* LineDir[0];
		end = V + (T2 - RelCloEnd2.x())*LineDir[1] + RelCloEnd2.y()* buw::createRotationZ22d(sig* M_PI / 2)* LineDir[1];
		...
		if (width > 0)
		{
			...
			{
				double start_angle = buw::calculateAngleBetweenVectors(x_axis, -LineDir[i]);
				...
				buw::vector2d p_endhelp = width*buw::createRotationZ22d(start_angle + sig* (M_PI / 2 + tau[i]))*x_axis;
