	...
	for (int Groupnumber = 0; Groupnumber < points_.size() - 2; Groupnumber++)
	{
		if (ral == true)
		{
			give3Points(P[Groupnumber * 3], P[Groupnumber * 3 + 1], P[Groupnumber * 3 + 2], LineDir[Groupnumber * 2], LineDir[Groupnumber * 2 + 1],
				beta[Groupnumber], t1[Groupnumber], t2[Groupnumber], points_[Groupnumber].xy(), points_[1 + Groupnumber].xy(), points_[2 + Groupnumber].xy(), offset_);
		}
		...
		center2 = end2 + R[2] * buw::createRotationZ22d(-sig*M_PI / 2)* LineDir[Groupnumber * 2 + 1];
