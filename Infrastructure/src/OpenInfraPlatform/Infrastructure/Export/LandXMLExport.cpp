...
namespace OpenInfraPlatform
{
	...
	{
		void exportAsLandXML(buw::DigitalElevationModel::Ptr digitalElevationModel, buw::AlignmentModel::Ptr alignmentModel, const std::string &filename)
		{
			QDomDocument doc;
			...
			for (int i = 0; i < digitalElevationModel->getSurfaceCount(); i++)
			{
				...
				buw::Surface::Ptr currentSurface = digitalElevationModel->getSurface(i);
			...
			for (int i = 0; i < alignmentModel->getAlignmentCount(); i++)
			{
				...
				buw::Alignment2DBased3D::Ptr alignment3d =
					std::static_pointer_cast<buw::Alignment2DBased3D>(alignmentModel->getAlignment(i));
				...
				if (alignment3d->hasHorizontalAlignment())
				{
					...
					{
						buw::HorizontalAlignmentElement2D::Ptr he = alignment3d->getHorizontalAlignment()->getAlignmentElementByIndex(hei);
