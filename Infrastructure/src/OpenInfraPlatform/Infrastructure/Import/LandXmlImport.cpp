	...
	namespace Infrastructure
	{
			...
			const std::vector<profAlignElement>& profAlignElements_)
		{
			...
			for (int i = 2; i < profAlignElements_.size(); i++)
			{
				...
				else if (b.type == eProfAlignType::CircCurve)
				{
					...
					buw::vector2d ab = buw::normalize(b.position - a.position);
					buw::vector2d bc = buw::normalize(c.position - b.position);

					buw::matrix22d rotation = buw::createRotationZ22d(-buw::constantsd::pi() / 2.0);
				...
				else
				{
					...
					if (start.x() > pvc.x())
					{
						double l = (start - pvc).length();
						int i = 0;
					}

					//if (start == pvc)
					if ((start - pvc).length() < 0.0001) // hack
					{
						// ignore element....	
						BLUE_LOG_STREAM_EX("LandXMLImport", buw::eLogSeverityLevel::Warning) <<
							"Detected invalid connecting element.";
					}
					else
					{
		...
		bool QLandXmlParser::startElement( const QString & /* unused */, const QString &name, const QString& /* unused */, const QXmlAttributes &attr )
		{
			...
			if (name == "CrossSectSurf")
			{
				...
				currentCrossSectSurface_=std::make_shared<CrossSectionSurface>();
			}

			if (name == "DesignCrossSectSurf")
			{
				pushState(eLandXMLParserState::DesignCrossSectSurf);
				currentDesignCrossSectSurface_=std::make_shared<DesignCrossSectionSurface>();
		...
		bool QLandXmlParser::endElement( const QString & /* unused */, const QString &name, const QString& /* unused */ )
		{
			...
			if (name == "CrossSectSurf")
			{
				...
				currentCrossSect_->addCrossSectionSurface(currentCrossSectSurface_);
			}
			...
			if (name == "DesignCrossSectSurf")
			{
				...
				currentCrossSect_->addDesignCrossSectionSurface(currentDesignCrossSectSurface_);
			}
			...
			if(name == "Spiral")
			{
				...
				else
				{
					...
					if (errX > 0.01 || errY > 0.01)
						BLUE_LOG_STREAM_EX("LandXML import: ", buw::eLogSeverityLevel::Error) << "clothoid is invalid";
				}
			}		
