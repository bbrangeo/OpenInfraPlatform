...
std::vector<std::string> OpenInfraPlatform::Infrastructure::OSMImport::getDefaultFilter()
{
...
std::vector<std::string> OpenInfraPlatform::Infrastructure::OSMImport::getModeNames()
{
...
OpenInfraPlatform::Infrastructure::OSMImport::OSMImport(const std::string& filename)
	:OSMImport(filename, getDefaultFilter(), 2)
{
}

OpenInfraPlatform::Infrastructure::OSMImport::OSMImport(const std::string& filename, const std::vector<std::string>& filter, int mode)
	: Import(filename)
{
			...
			if (!way.reverse)
			{
				for (auto it = way.wayPtr->nodeRefs.begin() + offset; it != way.wayPtr->nodeRefs.end(); it++)
				{
					...
					if (nodePtr)
					{
						...
						alignment->addPoint(buw::vector3d(p - centerOffset, 0));
					}
				}
			}
			else
