...
#if defined(LINUX) || defined(LINUX_64)
void cond_exit(int i)
{
    if (i != 1)
        exit(i);
};
#define fatal(a, b) cond_exit(1)
#define bfd_fatal(a) cond_exit(1)
#define bfd_nonfatal(a) cond_exit(1)
#define list_matching_formats(a) cond_exit(1)
#else
...
static char** translate_addresses_buf(bfd * abfd, bfd_vma *addr, int naddr)
{
	...
	char b;
	int total  = 0;
	enum stateType { Count = 0 , Print = 1 } state;
	char *buf = &b;
	int len = 0;
	char **ret_buf = NULL;
	 ...
	 * the second time we do the actual printing */
        int enu(0);
	for (enu=(int)Count; enu<=(int)Print; enu++) {
        state = (stateType)enu;
	if (state == Print) {
		...
		size_t size, void *data)
{
	...
	for (n = info->dlpi_phnum; --n >= 0; phdr++) {
		if (phdr->p_type == PT_LOAD) {
			...
			if ((ElfW(Addr))match->address >= vaddr && (ElfW(Addr))match->address < vaddr + phdr->p_memsz) {
				/* we found a match */
...
char **backtrace_symbols(void *const *buffer, int size)
{
	...
	for(x=stack_depth, y=0; x>=0; x--, y++){
		...
		addr = (bfd_vma)buffer[x] - (bfd_vma)match.base;
